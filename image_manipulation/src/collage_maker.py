...
ASSET_DIR = os.path.join(BASE_DIR, '..', '..', 'assets')   # 2 klasör yukarı -> ana assets klasörü
OUTPUT_DIR = os.path.join(BASE_DIR, '..', 'outputs')

# === Görselleri oku ===
img1 = cv2.imread(os.path.join(ASSET_DIR, 'img1.jpg'))
img2 = cv2.imread(os.path.join(ASSET_DIR, 'img2.jpg'))
img3 = cv2.imread(os.path.join(ASSET_DIR, 'img3.jpg'))
img4 = cv2.imread(os.path.join(ASSET_DIR, 'img4.png'))

# Görsellerin kontrolü
for i, img in enumerate([img1, img2, img3, img4], start=1):
    if img is None:
        print(f"Görsel {i} okunamadı! Dosya adını veya uzantısını kontrol et.")
        exit()
...
# === Renk dönüşümleri (hepsi BGR olacak şekilde) ===
...
output_path = os.path.join(OUTPUT_DIR, 'collage_out.jpg')
cv2.imwrite(output_path, collage)
print(f"Kolaj başarıyla kaydedildi: {output_path}")
